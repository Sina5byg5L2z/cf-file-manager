...
- **沉浸式看图**：图片预览为独立看图模式——滚轮 / 双击 / 双指捏合缩放、拖拽平移、旋转翻转、同目录画廊左右切换（底部缩略图条 + 相邻预加载）、幻灯片播放、黑 / 白 / 棋盘背景切换、浏览器全屏；SVG 走图片方式渲染不注入页面（脚本不可执行）
- **图床**：外链图片托管，支持公开访问 `/i/*`，可从文件管理器导入；上传支持断点续传与取消
- **上传前压缩**：上传图片 / 视频时弹窗可选目标分辨率与画质，压缩全部在浏览器本地完成（视频走 WebCodecs 硬件加速，不行再降级 MediaRecorder），服务端不参与、不耗配额；可一键跳过直接传原文件，也可在设置里整个关闭
...
- **参数设置**：分片规则、上传上限、预览上限、直下阈值、上传前压缩页面内可视化调整，按移动端 / 电脑端两档分别生效
...
| 直下阈值 | 不超过此大小的文件下载走浏览器原生下载（快、可续传、无校验），超过走分片校验下载；只允许固定档位 | 4MB（档位 0 / 1 / 2 / 4 / 8 / 16 / 32 / 48MB） |
| 上传前压缩 | 开关 + 图片长边档位（原尺寸~854px）/ 视频短边档位（原尺寸~360p）/ 画质三档（高/中/低）。压缩只在浏览器本地做，默认开启弹窗；关掉后不弹窗直接传原文件 | 开启（保持原尺寸，画质高） |
| 歌词 | 歌词来源（自动 / 仅 LRCLIB / 仅 lrc.cx / 关闭）、译文与罗马音来源（网易云，需自部署 API 地址）、网易云 API 地址（私有字段，公开接口不下发，仅登录后回显） | 自动来源；译文关闭 |
| AI 翻译 | 开关 + 接口地址 / 模型 / 密钥（OpenAI 兼容，默认硅基流动 Qwen3-8B）。密钥属私有字段，留空时回落部署期环境变量兜底；关闭或未填密钥时前端不出现翻译按钮 | 关闭 |
...
- 小技巧：上传时遇到分片上传失败，且重试也失败的"走投无路"情况，可以关掉浏览器重新打开，点击右下方的上传按钮重新选择文件，可以续传。

### 大文件下载

- 页面内下载（文件管理器 / 分享页）：大于「直下阈值」的文件自动走分片下载（3 并发），带三重完整性校验（响应声明总长、每段字节数、拼装总长）+ 单段失败折半窗口重试（最小 1MB），校验不过直接报错，绝不保存残缺文件；阈值内的走浏览器原生下载（快、可断点续传，但无校验）。直下阈值在「参数设置」里调，固定档位 0 ~ 48MB（默认 4MB；48MB 压在平台单响应 CPU 掐断点之下，档位内直下不会被静默截断）
- 无 JS 的直链下载（WebDAV / curl / 右键另存为）：超过 32MB 会明确返回 413 报错而非静默截断（平台上整文件流式响应超 CPU 上限会被掐断，客户端无感，宁可报错）；≤32MB 仍可直链下载

## 音乐播放器与歌词

音频文件点击即可播放。管理页与分享页用的是同一个播放器（播放队列 / 歌词 / 封面一致；分享页为只读，不能改歌曲信息、不能拉黑）。默认开箱即用（在线歌词源免费无鉴权），不需要时在「参数设置 → 歌词」关闭。

- **歌词**：优先用你手动粘贴的歌词；没有时自动按 **LRCLIB → lrc.cx** 依次获取。在线取到的歌词会持久保存，管理页与分享页共用同一份；拉黑来源或修改歌曲信息后自动失效重取。某次配上的歌词是错的，可一键**拉黑该来源**（按文件记录，随时撤销）降级重取，全部拉黑则显示「暂无歌词」；拉黑 / 换源结果会清掉旧缓存立即生效。歌词字号可调（T+ / T−）
- **歌曲信息**：标题 / 歌手 / 专辑可手动编辑（优先于音频内嵌标签与文件名解析）；歌词整体快了 / 慢了可微调偏移（±30 秒）
- **译文 / 罗马音**：目前只有网易云提供，可选项。需自部署 NeteaseCloudMusicApi，把 API 地址填进设置即可（属私有字段，未登录的任何接口都拿不到它）
- **AI 翻译**：默认关闭。在「参数设置 → 歌词」里打开并填好密钥后，歌词面板会出现「AI 翻译」按钮，把带时间轴的原文逐行翻成简体中文，生成原文 / 译文双语对照；结果等同于手填译文，可随意编辑或删除，翻不出的行保留原文。任何 OpenAI 兼容接口都能用（默认硅基流动 `Qwen/Qwen3-8B`，地址 / 模型 / 密钥均可改），密钥与 API 地址一样属私有字段，未登录拿不到；AI 翻译不消耗 Cloudflare 配额之外的东西，只按你选的模型商计费
- **封面**：网易云（配置了 API 才启用）→ Deezer → iTunes 依次尝试，只返回图片直链不代理字节，不消耗 Worker 流量
...
src/         Worker 后端（纯原生 fetch handler，无框架）
  ...
  settings.js  应用参数设置（分片规则 / 上传与预览上限 / 下载窗口 / 直下阈值 / 压缩 / 歌词，D1 单行存储）
  storage.js   存储分库路由（库注册表 / 选库 / 容量告警 / 注册与启用接口）
  blobops.js   跨库字节操作与补偿重试
  lyrics.js    歌词代理（LRCLIB / lrc.cx / 网易云）与专辑封面直链
  trackmeta.js 歌曲元数据（手动标题 / 歌手 / 歌词覆盖，path 主键）
