...
音频文件点击即可播放，管理页与分享页均支持。默认开箱即用（在线歌词源免费无鉴权），不需要时在「参数设置 → 歌词」关闭。

- **歌词**：优先用你手动粘贴的歌词；没有时自动按 **LRCLIB → lrc.cx** 依次获取。某次配上的歌词是错的，可一键**拉黑该来源**（按文件记录，随时撤销）降级重取，全部拉黑则显示「暂无歌词」；拉黑 / 换源结果会清掉旧缓存立即生效
